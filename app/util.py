...
def convert_name_to_image_filename(name):
    name = name.decode('utf-8')
    filename = ''.join([converter.convert(n) for n in name])
    filename += '.jpg'
    return filename
...
def format_person_info(dict):
    ...
    image_file_name = convert_name_to_image_filename(name)

    image_path = os.path.join(student_image_dir,image_file_name)
    if os.path.exists(image_path):
        image_path = os.path.join('pic/student',image_file_name)
    else:
